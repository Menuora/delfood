document.addEventListener('DOMContentLoaded', async () => {
  ...
  try { const data = await (await fetch('/api/images')).json(); const images = data.images || []; render(menu, images.filter(i => i.type === 'menu')); render(items, images.filter(i => i.type === 'item')); } catch { render(menu, []); render(items, []); }
});
